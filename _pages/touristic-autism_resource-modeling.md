...
このアプリで実現したいことは何でしょうか？まずは以下を実現します。
...
これらの要件は、user、place、comment、ratingという4つの異なるリソースを識別するのに役立つことに注目してください。これから、それぞれのプロパティやお互いの関連を定義することで、それらをモデル化していきます。
...
{% coach %}
作成されたUserモデルについて説明してください。フィールドとは何でしょうか？モデルは `ActiveRecord::Base` というスーパークラスからDBと対話する能力を継承していることに注目してください(参照: MVC)。
{% endcoach %}
...
最後に、ユーザーがログインしていない場合に強制的にログインページにリダイレクトするようにします。`app/controllers/application_controller.rb` を開き、以下のコードを
...
変更をGitHubにadd、commit、pushしましょう!すべての変更がうまくGitHubのプロフィールに反映されているのがわかりますか？ :)

## 観光地

Railsのscaffold機能を使用して、2つめのリソース "place" を一覧表示、作成、削除、変更、表示するために必要なものすべてを作成できます。
...
私たちはすでにデータベースを使用しています(Gemfileに `gem 'sqlite'` が記述されています)。次のコマンドを実行して、"place" 構造体をテーブルとしてデータベースに追加しましょう。
...
画像や名前などのフィールドと同様に、"place" の新しいインスタンスごとに、一意の識別子 "primary key" が自動的に割り当てられるため、開発者が特に指定する必要はありません。

{% coach %}
Railsのscaffoldとは何でしょうか? migrationとは何で、なぜ必要なのでしょうか？
{% endcoach %}

"place" リソースを操作するために作成されたページとその命名規則に注目してください。
サーバーのログを確認して、以下のやり取りがどのように処理されているのかを説明してください(MVCパターンの観点で)。

* ブラウザは /places URL へのリクエストを行う
* Railsは /places を Places コントローラの index アクションにルーティングする
* index アクションは Place モデルにすべてのplaceを取得するように依頼します(Place.all)
* Place モデルはデータベースからすべてのplaceを取得します。
* Place モデルはplaceの一覧をコントローラに返します。
* コントローラはユーザーを @users 変数に保存し、index ビューに渡します。
...
app/models/user.rb を開いて、次のコードの後に
...
app/views/places/_form.html を開いて、次のコードの後に
...
app/views/places/index.html.erb を開いて、次のコードを
...
app/models/comment.rb を開いて、次のコードの後に
...
app/views/comments/_form.html.erb を開いて、次のコードを
...
app/views/places/show.html.erb を開いて、一番下のリンクの直前に下記のコードを追加してください
...
app/controllers/places_controller.rb を開いて、showアクション内の次のコードの後に
...
app/views/places/show.html.erb を開いて、下記のコードを
...
## リソースのフィールドに対するバリデーション

現時点では、comment、place、userはその情報が正しいかどうかの検証が行われていません。とはいえ、例えば、レビューのcommentの長さの制限やuserのemailアドレスのフォーマットには制限があるべきです。

commentのbodyフィールドに長さの制限を追加しましょう(validatesを使用します)。
app/models/comment.rb を開いて、'class' と 'end' の間に下記のコードを追加しましょう。
