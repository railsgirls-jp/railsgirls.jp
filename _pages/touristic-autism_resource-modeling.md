...
私たちは、このアプリに何をしてほしいのでしょうか？まず最初に、以下のことを実現したいと考えています。
...
`app/views/layouts/application.html.erb`を開いて、追加します。
...
これで準備が整ったので、最初のユーザーを作成できます。deviseはアカウントの作成、ログイン、ログアウトなどに必要なすべてのコードとルーティングを生成します。
...
変更をGitHubにadd,commit,pushしましょう!すべての変更がうまくGitHubのプロフィールに反映されているのを確認しましょう:)

## Touristic Place モデル

Railsのscaffold機能を使用して、2つめのリソース"touristic place"を一覧表示、作成、削除、変更、表示するために必要なものすべてを作成できます。
...
`scaffold`はプロジェクトディレクトリに新しいファイルを作成します。しかし、私たちは構造を定義(モデル化)しており、今後作成されるこのリソースのインスタンスがその構造に従い、どこか、つまり、データベースに保存したいと考えています。
私たちはすでにデータベースを使用しています。(Gemfileに`gem 'sqlite'`を参照してください)。次のコマンドを実行して、"place"構造体をテーブルとしてデータベースに追加しましょう。
...
その後、もう一度サーバーを起動させましょう。[http://localhost:3000/places](http://localhost:3000/places)にアクセスして、"place"を扱うためのすべての機能があることを確認しましょう。Ruby on Railsの`generate scaffold`が自動生成してくれたおかげです。
データベースに保存される"place"の新しい各インスタンスには、"primary key"と呼ばれる一意の識別子が自動的に割り当てられます。これは、私たちがpictureやnameなどと一緒にフィールドの1つとして指定する必要はありません。
{% coach %}
Railsのscaffoldとは何でしょうか? migrationとは何で、なぜ必要なのでしょうか？
{% endcoach %}

"place"リソースを操作するために作成されたページとその命名規則に注目してください。サーバーログを見て、次のやり取りについてレポートとして説明してください。(MVCパターンの文脈で)
...
現時点では、comments、places、usersは、その正しさが一切検証されていない情報によって構成されています。それでも、例えばreview内のcommentsの長さの制限やuserのemailアドレスの形式には制限があるべきです。
