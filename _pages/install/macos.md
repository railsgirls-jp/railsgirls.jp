...
rbenv がインストールされたので、Ruby をインストールしましょう！バージョン 3.4.7 をインストールしますが、イベントでは他のバージョンをインストールするようにお願いされるかもしれません。その場合、コマンドに含まれるバージョン番号を単純に入れ替えてください。
...
対象バージョン（例: 3.4.7）が表示されていない場合、下記コマンドを実行してみましょう。
{% highlight sh %}
