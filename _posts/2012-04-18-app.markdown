...
* ターミナル (Terminal; Windowsではコマンドプロンプトと言う) は、rails serverを起動したりコマンドを実行したりするものです。
...
Terminal 上で CTRL-C を実行してサーバを終了します。(WindowsでCTRL-Cで終了しない場合は、CTRL-PAUSEを試してください。)
...
さて、写真を処理するコードを生成します。Terminal で、次のコマンドを実行してください。 :

{% highlight sh %}
bundle
{% endhighlight %}

**ここで、Terminal で動いている rails server プロセスをリスタートする必要があります。** 追加した gem をアプリにロードさせるためです。
...
OS X と Linux は、Terminal で次のコマンドを実行します。 :
