...
JavaScript 関連の問題を解決するため、`C:\RailsInstaller\Ruby1.9.3\lib\ruby\gems\1.9.1\gems\execjs-2.0.2\lib\execjs\runtime.rb` を Sublime Text 2 などのテキストエディタで開いて下さい。そして、41行目からの
...
を次のように変更して、保存して下さい。（`:command` と `:encoding` の部分）
