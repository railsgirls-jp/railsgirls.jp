...
~/.ssh ディレクトリ内のファイルが一覧で表示されますので `xxx.pub` という公開鍵ファイルがあるかを確認しましょう。
デフォルト設定でつくられている場合、公開鍵ファイルは次のような名前になっています。
- id_rsa.pub
- id_ecdsa.pub
- id_ed25519.pub
...
すでに `id_ed25519` が存在する場合や別のファイルにしたい場合は、任意のファイルを指定しましょう。
