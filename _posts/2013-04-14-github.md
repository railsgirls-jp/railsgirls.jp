...
すでに SSH キーを持っているかを確認します。ターミナルで以下のように入力してください。

{% highlight sh %}
ls -al ~/.ssh
{% endhighlight %}

~/.ssh ディレクトリ内のファイルが一覧で表示されますので、
`id_rsa.pub` や `xxx.pub` のような公開鍵があるかを確認しましょう。

#### SSH キーを生成する

（既存の SSH キーを使う場合は、この手順はスキップしてください。）

SSH キーが存在しなかった場合、または、持っているキーとは別の SSH キーを生成したい場合には新しく SSH キーをつくります。

ターミナルで以下のコマンドを実行してください。（your_email@example.com の部分にはあなたのメールアドレスが入ります。）
...
キーを保存するファイルを聞かれます。
デフォルトの `~/.ssh/id_rsa` で問題なければ何も入力せずに Enter を押します。
すでに `id_rsa` が存在する場合や別のファイルにしたい場合は、任意のファイルを指定しましょう。
...
（任意のファイルに SSH キーを生成した場合は、`~/.ssh/id_rsa.pub` の部分を読み替えてください。）
